...
class KnockOutFixture:
    ...
    def initialBracket(self, fixture_id):
        ...
        category_instance = fixture_instance.category
        teams = [team.id for team in category_instance.teams.all()]
        teams_names = [team.name() for team in category_instance.teams.all()] # [ team names ]
        n = len(teams)
        nearest_power_of_2 = 2 ** math.ceil(math.log2(n))
        byesNeeded = nearest_power_of_2 - n
        ...
        # makelvl JSON
        ttl_grps = []
        self.makeLvlJSON(lvl_grp, cur_lvl, ttl_grps, teams_names, fixture_id)
    ...
    def makeLvlJSON(self, lvl_grp, cur_lvl, ttl_grps, teams):
        ...
        for i, level in enumerate(lvl_grp):
            ...
            if teams is not None:
                ...
                ttl_grps.append({"key": level, "parent": parent, "player1": p1, "player2": p2, "parentNumber": parent_number})
            else:
                ttl_grps.append({"key": level, "parent": parent, "parentNumber": parent_number})
            parent_number = (parent_number + 1) % 2
            
        if cur_lvl >= 0:
            self.make_lvl(parent_keys, cur_lvl, ttl_grps, None)
        else:
            print("done, check ttl_grps is updated or not...")
    ...
    def nextBracket(self, fixture_id):
        ...
        fixture_instance.currentWinners.clear()
        winners_match = [[entry.get('player1', ''), entry.get('player2', '')] for entry in fixtureJSON if entry['key'].startswith(f'{currentStage}-')]
        print("teams", winners_match)
    ...
    def updateWinnerJSON(self, winner, fixture_id):
        ...
        for match in fixtureJSON:
            ...
            if lvl == cur_lvl:
                if match.get('player1') == winner or match.get('player2') == winner:
                    found = True
                    break
        if not found:
        ...
        for parent_node in fixtureJSON:
            if parent_node['key'] == parent_key:
                parent_node[f'player{m_key}'] = winner
                break
